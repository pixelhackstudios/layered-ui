...
- **Native HTML First**: Simple and intermediate components (`LayeredButton`, `LayeredInput`, `LayeredSelect`, `LayeredDisplayCard`, `LayeredPanel`, `LayeredTextarea`, `LayeredCheckbox`, `LayeredSwitch`, `LayeredNumberField`, `LayeredBadge`) use semantic HTML elements (`<button>`, `<input>`, `<select>`, `<section>`, `<textarea>`, `<input type="checkbox">`, `<input type="number">`, `<span>`) to preserve native browser semantics, keyboard handling, and form integration where applicable.
- **Radix Primitives for Complex Behavioral Controls**: Radix primitives are approved *exclusively* for complex behavioral controls where native HTML lacks standard accessible lifecycle management (such as overlay focus traps, portal mounting, escape key handling, roving focus, and ARIA state management). `LayeredDialog` was the first implemented example, built on `@radix-ui/react-dialog` (pinned at `1.1.23`). `LayeredTooltip` is the second, built on `@radix-ui/react-tooltip` (pinned at `1.2.16`), covering hover/focus delay timing, portal mounting, collision-aware positioning, and dismissal. `LayeredTabs` is the third, built on `@radix-ui/react-tabs` (pinned at `1.1.21`) — the first non-overlay Radix component. Its justification is different from Dialog/Tooltip's: Tabs has no portal or overlay lifecycle to manage; the native-HTML gap it fills is roving tabindex, arrow-key/Home/End keyboard navigation, activation-mode semantics, and the ARIA tab/tabpanel relationship, none of which native HTML provides out of the box for a custom-styled tab rail. `LayeredAccordion` is the fourth, built on `@radix-ui/react-accordion` (pinned at `1.2.20`) — the second non-overlay component, for the same class of reason as Tabs: the native-HTML gap it fills is single/multiple disclosure state management, roving-focus keyboard navigation (Up/Down/Home/End), collapsible semantics, and the heading/button/region ARIA relationships a hand-rolled disclosure widget would otherwise have to reimplement. `LayeredToast` is the fifth, built on `@radix-ui/react-toast` (pinned at `1.2.23`) — the third overlay-based component, after Dialog and Tooltip. The native-HTML gap it fills is transient-notification lifecycle management: auto-dismiss timing, pause-on-hover/focus/window-blur, foreground/background ARIA live-region announcement priority, and swipe-gesture dismissal state — none of which native HTML provides for a custom-styled toast. `LayeredPopover` is the sixth, built on `@radix-ui/react-popover` (pinned at `1.1.23`) — the fourth overlay-based component, after Dialog, Tooltip, and Toast. The native-HTML gap it fills is anchored-overlay lifecycle management: Trigger/Anchor-relative positioning, collision detection and side/align flipping, portal mounting, focus containment, and outside-interaction/Escape dismissal — the same class of gap Tooltip fills, but for interactive rather than read-only content, which is why Popover also needs `modal`/non-modal focus behavior that Tooltip never required. `LayeredRadioGroup` is built on `@radix-ui/react-radio-group` (pinned at `1.4.7`). Unlike Dialog/Tooltip/Toast/Popover, it is not overlay-based (no portal, no z-index token), the same class as Tabs/Accordion; the native-HTML gap it fills is roving-tabindex arrow-key navigation across a group of otherwise-independent radio inputs and the `role="radiogroup"`/`role="radio"` ARIA relationship, which native `<input type="radio">` provides implicitly for unstyled radios but not when each radio's visible surface is a fully custom casing. `LayeredSlider` is built on `@radix-ui/react-slider` (pinned at `1.4.7`) — the ninth Radix-backed component and the third non-overlay one, the same class as Tabs/Accordion/RadioGroup. The native-HTML gap it fills is pointer-drag value tracking across an arbitrary custom-styled rail, keyboard step/page/Home/End navigation, `minStepsBetweenThumbs` collision prevention for multi-thumb range selection, and the `role="slider"`/`aria-valuenow` relationship — none of which a hand-styled native `<input type="range">` can provide once its default UA appearance is fully replaced (only one native thumb, no independently styleable track-fill segment, no multi-thumb range support). `orientation` is omitted and forced to `horizontal`, deliberately horizontal-only in v1 — the same class of trim as Accordion's vertical-only decision.
- **Native Progress Semantics**: `LayeredProgress` uses native `<progress>` semantics and has no behavioral dependency. A visually hidden native element carries its value/max or indeterminate state while an aria-hidden channel supplies the consistent constructed appearance; no duplicate progressbar role is introduced.
- **Native Number Field Semantics**: `LayeredNumberField` uses `<input type="number">` for numeric text entry, form integration, min/max/step constraints, and ArrowUp/ArrowDown keyboard stepping. Its attached buttons delegate to native `stepUp()`/`stepDown()` behavior. Base UI Number Field was evaluated but not adopted because v1 does not require its locale formatting, scrubbing, or compound-part behavior.
- **Native Badge Semantics**: `LayeredBadge` renders a non-interactive `<span>` and introduces no role, focusability, or behavior. It generalizes `LayeredDisplayCard`'s compact status treatment as a standalone primitive while leaving the card implementation unchanged.
- **Undecided Behavioral Primitives**: `LayeredCombobox`'s behavioral dependency was resolved to Base UI's Combobox primitive (see above), not Radix — it is not grouped under the approved Radix strategy. Existing Radix and Base UI adoption and the native implementations of `LayeredProgress` and `LayeredNumberField` do not predetermine the primitive choice for a future `LayeredAlertDialog`; each component is evaluated independently against what native HTML can and cannot provide.
- **Compound APIs**: `LayeredDialog` was the first component to expose a compound export API (`LayeredDialog`, `LayeredDialogTrigger`, `LayeredDialogContent`, `LayeredDialogTitle`, `LayeredDialogDescription`, `LayeredDialogClose`, `LayeredDialogHeader`, `LayeredDialogFooter`). This established that compound APIs are permitted when a component's behavior genuinely requires composition — it is not a predetermined template for future components. `LayeredTooltip` confirms this: it exposes its own, smaller four-part shape (`LayeredTooltipProvider`, `LayeredTooltip`, `LayeredTooltipTrigger`, `LayeredTooltipContent`), with no separate `LayeredTooltipArrow` or `LayeredTooltipPortal` export — the directional arrow is rendered internally by `LayeredTooltipContent` behind a `showArrow` prop, since it is a fixed visual detail of one surface rather than something consumers need to compose freely. `LayeredTabs` exposes its own four-part shape (`LayeredTabs`, `LayeredTabsList`, `LayeredTabsTrigger`, `LayeredTabsContent`), with no separate indicator export — the active-tab/content relationship is expressed through per-element styling, not a free-floating element consumers would need to compose. `LayeredAccordion` exposes a parallel four-part shape (`LayeredAccordion`, `LayeredAccordionItem`, `LayeredAccordionTrigger`, `LayeredAccordionContent`) with no separate `LayeredAccordionHeader` export — `Accordion.Header` is always rendered internally by `LayeredAccordionTrigger` around a heading element chosen via a `headingLevel` prop (`2`–`6`, default `3`), since it's a fixed semantic wrapper with no meaningful independent composition, not a togglable visual detail like Tooltip's arrow. `LayeredToast` exposes a seven-part shape (`LayeredToastProvider`, `LayeredToastViewport`, `LayeredToast`, `LayeredToastTitle`, `LayeredToastDescription`, `LayeredToastAction`, `LayeredToastClose`) — one part more than Tooltip's four but, notably, **no Trigger part**: unlike Dialog/Tooltip (opened by a consumer clicking/hovering a rendered trigger element), a toast is opened by consumer application state (a mutation succeeding, a background job finishing), so there is no click target for Layered UI to wrap. See "`LayeredToast` Compositional Boundary" below for the full API rationale. `LayeredPopover` exposes a five-part shape (`LayeredPopover`, `LayeredPopoverTrigger`, `LayeredPopoverAnchor`, `LayeredPopoverContent`, `LayeredPopoverClose`), between Tooltip's four and Dialog's eight — it needs `Anchor` (Tooltip doesn't: a Popover consumer may position Content against an element other than its click Trigger, or control Root and compose Anchor with no Trigger at all), but has no Title/Description/Header/Footer, because `@radix-ui/react-popover` does not expose Radix `Title`/`Description` primitives the way `@radix-ui/react-dialog` does, and Popover content is free-form rather than a fixed title-plus-body-plus-actions structure. See "`LayeredPopover` API Boundary" below for the full rationale, including why an earlier planning draft's assumption of Radix-backed `Title`/`Description` exports was verified against the actual installed package and found incorrect before implementation. `LayeredRadioGroup` exposes a two-part shape (`LayeredRadioGroup`, `LayeredRadioGroupItem`), the smallest compound API published so far — smaller than Tabs'/Accordion's four parts because Item has no separate Trigger/Content split (a radio button is its own single interactive+visual unit), and no separate `LayeredRadioGroupIndicator` export — `Radio.Indicator` is not rendered at all; checked state reads Radix's own `data-state="checked"` on Item directly to light the face, so there is no indicator dot needing a home, exported or internal. Root owns `tone`/`radioGroupSize`, following Tabs'/Accordion's Root-owned precedent rather than Tooltip's/Popover's Content-owned one, since Item (unlike Popover's Content) is a direct DOM descendant of Root with no Portal boundary to cross. `LayeredSlider` exposes a four-part shape (`LayeredSlider`, `LayeredSliderTrack`, `LayeredSliderRange`, `LayeredSliderThumb`) mirroring Radix's own Root/Track/Range/Thumb primitive names exactly, with no separate indicator export — same as Tabs, the active-value relationship is expressed through Range's fill and Thumb's position, not a free-floating element consumers would need to compose. Root owns `tone`/`sliderSize`, following Tabs'/Accordion's/RadioGroup's Root-owned precedent, since Track and Thumb are Root siblings. Unlike Tabs/Accordion (fixed Trigger/Content counts) or RadioGroup (one Item per option), Slider's Thumb count is consumer-determined — one `LayeredSliderThumb` per entry in `value`/`defaultValue`, so a two-thumb range slider is composed by rendering two Thumbs, the same free composition raw Radix already permits. `LayeredNumberField` returns to a single component export because its native numeric input and fixed attached actuators do not require consumer composition. None of these component-specific decisions predetermine the API of a future component such as `LayeredAlertDialog`, which remains undecided.
