...
The canonical manifest (`registry.json`) defines nineteen published items:
...
| `layered-radio-group` | `registry:ui` | `registry/components/layered-radio-group/LayeredRadioGroup.tsx`<br>`registry/components/layered-radio-group/LayeredRadioGroup.css` | `public/r/layered-radio-group.json` | `pixelhackstudios/layered-ui/layered-foundation` (registry); `@radix-ui/react-radio-group@1.4.7` (npm) |
...
`layered-radio-group` is the ninth, and the eighth Radix-backed component, built on `@radix-ui/react-radio-group`, pinned exactly at `1.4.7`, verified via `npm view @radix-ui/react-radio-group@1.4.7 peerDependencies` (confirms `react`/`react-dom` `^19.0` support). Unlike `LayeredCheckbox`/`LayeredSwitch`, which hide a native `<input>` behind a fully custom-styled label, `LayeredRadioGroupItem` styles Radix's `RadioGroup.Item` directly as the visible casing — Item is already a real, focusable `<button role="radio">`, so there is no native control left to hide. Checked state is communicated purely by the face lighting up (a tone-colored radial-gradient glow keyed off Radix's own `data-state="checked"` on Item) — no separate indicator dot is rendered, and `Radio.Indicator` is not used at all, an earlier pass that rendered a small centered dot on top of the glow was removed as visually redundant. `LayeredRadioGroupIndicator` is not separately exported.

`layered-slider` is the tenth item with an external behavioral primitive and the ninth Radix-backed component, built on `@radix-ui/react-slider`, pinned exactly at `1.4.7`. It supplies pointer-drag tracking, keyboard stepping, multi-thumb range behavior, and slider ARIA state. The Layered wrapper deliberately supports horizontal orientation only in v1 and exposes Root, Track, Range, and Thumb parts; consumers render one Thumb for each entry in `value` or `defaultValue`.
